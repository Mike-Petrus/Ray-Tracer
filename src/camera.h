...
class camera {
    ...
    private:
        ...
        void initialize() {
            ...
            // Calculate the u, v, w unit vectors for the camera coordinate frame
        ...
        ray get_ray(int i, int j) const {
            // Construct a camer ray originating from the -defocus disk- and directed at a randomly sampled
            // point around the pixel i, j

            vec3 offset = sample_square();
            vec3 pixel_sample = pixel_00_loc + ((j + offset.x()) * pixel_delta_u) + ((i + offset.y()) * pixel_delta_v);
        ...
        color ray_color(const ray& r, int depth, const hittable& world) const {
            // If we've hit the ray bonce limit, no more light is gathered
